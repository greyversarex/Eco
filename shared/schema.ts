...
// Visual Templates table (Намунаҳои визуалӣ) - templates with background image and positioned fields
export const visualTemplates = pgTable("visual_templates", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  description: text("description"),
  backgroundImage: bytea("background_image").notNull(), // PNG/JPEG image of template
  backgroundMimeType: text("background_mime_type").notNull(),
  pageWidth: integer("page_width").notNull().default(595), // A4 width in points (72dpi)
  pageHeight: integer("page_height").notNull().default(842), // A4 height in points
  fields: jsonb("fields").notNull().default([]), // Array of field definitions with positions
  isActive: boolean("is_active").default(true).notNull(),
  sortOrder: integer("sort_order").default(0).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
...
// Field definition type for visual templates
export const visualTemplateFieldSchema = z.object({
  id: z.string(),
  name: z.string(), // Field identifier like "number", "date.day"
  label: z.string(), // Display label in Tajik
  x: z.number(), // X position (percentage 0-100)
  y: z.number(), // Y position (percentage 0-100)
  width: z.number(), // Width (percentage)
  height: z.number(), // Height (percentage)
  fontSize: z.number().optional(), // Font size in pt
  fontFamily: z.string().optional(),
  textAlign: z.enum(["left", "center", "right"]).optional(),
  placeholder: z.string().optional(),
});

export type VisualTemplateField = z.infer<typeof visualTemplateFieldSchema>;

export const insertVisualTemplateSchema = createInsertSchema(visualTemplates).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
export type InsertVisualTemplate = z.infer<typeof insertVisualTemplateSchema>;
