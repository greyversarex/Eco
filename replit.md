...
- **Message Forwarding (Иловакунӣ):** Department users can forward messages to other departments via a "Фиристодан" button next to the reply button. The system tracks both the original sender (`originalSenderId`) and the person who forwarded the message (`forwardedById`). Forwarded messages display "Иловашуда: " prefix in the subject and show original sender and forwarder information in a highlighted info block. All attachments are automatically copied to the forwarded message. Only message senders or recipients have permission to forward messages.
...
- **Recycle Bin (Корзина) System:** Soft-delete functionality for messages, assignments, and announcements with department-scoped visibility. Each department sees only their own deleted items in trash. Admin panel includes department-specific trash view accessible from department cards with three sections (Ҳуҷҷатҳо/Messages, Супоришҳо/Assignments, Эълонҳо/Announcements). Each section displays count badges, complete metadata (sender, recipients, deadlines, deletion dates), and permanent delete capability (admin-only). Backend provides department-filtered endpoints (`/api/trash/messages/department/:deptId`, `/api/trash/assignments/department/:deptId`, `/api/trash/announcements/department/:deptId`) for admin access to specific department trash. Permanent deletion triggers immediate cache refresh for both trash and main lists.
