...
- **Department-Specific Assignments (Complete):** Transformed assignments from global to department-targeted by adding `recipientIds` field to schema. Backend filters assignments by recipientIds—departments see only their targeted assignments while admins see all. Created safe migration for `recipient_ids` column. Backward compatible: legacy assignments without recipients remain visible to all departments. Frontend updated with department recipient selection UI (matches ComposeMessage pattern) in AssignmentsPage. Special features for Рохбарият (ROHBAR001): assignment creation directly from messages via modal dialog, monitoring page with full access to all department messages, and enhanced oversight capabilities.
- **Рохбарият Special Features:** Added exclusive features for "Раёсати кадрҳо, коргузорӣ ва назорат" department (ROHBAR001): (1) "Вазифагузорӣ" button in MessageView to create assignments from received messages with pre-filled metadata; (2) "Назорат" monitoring button on DepartmentMain; (3) MonitoringPage at /department/monitoring showing all department messages like admin view, with proper access control using TanStack Query `enabled` flag to prevent unauthorized data access.
