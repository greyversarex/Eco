...
- **Admin Department Management Redesign (Complete):** Completely redesigned admin panel department management with modern card-based layout and drag-and-drop functionality. Features include: sortOrder field in departments schema for custom ordering, new `/api/departments/reorder` endpoint for batch updating order, SortableCard component using @dnd-kit library for smooth drag-and-drop, responsive grid layout (1/2/3 columns), click-to-edit functionality where clicking any card opens edit dialog, visual drag handles with hover effects, permission badges displayed on cards (Назорат/Вазифагузорӣ/Супоришҳо), optimistic UI updates for instant feedback. Cards show department name, block, access code with copy button, and action buttons (edit/regenerate/delete). Sorting now prioritizes custom sortOrder, then falls back to block hierarchy (Upper→Middle→Lower→District) and alphabetical order. Safe migration ensures sortOrder field addition without data loss.
