...
- **Recycle Bin (Корзина) System (Complete):** Implemented complete soft-delete functionality for messages and assignments. Features: `isDeleted` and `deletedAt` fields in database schema with indexes, all storage methods filter by deletion status, dedicated trash API endpoints (GET/POST for messages and assignments), TrashPage with two-tab interface displaying deleted items, restore functionality with proper cache invalidation, security measures ensuring departments can only restore their own items, navigation button in DepartmentMain with Trash2 icon. Items are never permanently deleted - they are only marked as deleted and can be restored at any time. Attachment downloads verify parent item deletion status for security.
- **Unified Department Ordering System (Complete):** Implemented centralized sortOrder-based department ordering that controls display across entire application. Admin panel drag-and-drop sets the order, which automatically propagates to DepartmentMain dashboard, ComposeMessage recipient selection, and AssignmentsPage recipient selection. Features: sortOrder field in departments schema (default 0), `/api/departments/reorder` endpoint for batch updates, departments sorted by sortOrder within each block across all pages, block-grouped display with headers (Боло/Миёна/Поён/Ноҳия) matching user dashboard aesthetic, visual separators between blocks, SortableCard component using @dnd-kit for smooth drag-and-drop, responsive grid layout (1/2/3 columns), click-to-edit functionality, permission badges on cards, optimistic UI updates. Backend getDepartments() returns pre-sorted list by sortOrder. This ensures consistent department ordering site-wide controlled from single admin interface.
- **People/Executors Management System (Complete):** Implemented comprehensive system for managing people (executors/иҷрокунандагон) with department associations. Features include: dedicated `people` table with department foreign key, full CRUD API endpoints, AdminPeople page for managing people, automatic filtering by selected departments in both ComposeMessage and AssignmentsPage forms. When creating messages or assignments, users select departments first, then see only people from those departments with visual grouping by department name. Replaced hardcoded EXECUTORS_LIST with dynamic database-driven system. Safe migration added to server/safe-migrate.ts for production deployment. **Initial seed data**: 46 executors automatically added to "Роҳбарият" department on first migration - this ensures all executors are present on both development and production servers after running `npm run db:migrate`.
- **Flexible Permission System (Complete):** Replaced hardcoded department checks with database-driven permission system. Added three boolean fields to Departments table: `canCreateAssignment` (create assignments/announcements), `canCreateAssignmentFromMessage` (Вазифагузорӣ button in message view), and `canMonitor` (access monitoring page). Admins control permissions via checkboxes in admin panel. Backend validates permissions from database instead of department names. Frontend conditionally renders features based on user.department permission flags. After permission changes, departments must logout/login to refresh session data. This enables flexible permission assignment to any department without code changes.
- **Department-Specific Assignments (Complete):** Transformed assignments from global to department-targeted by adding `recipientIds` field to schema. Backend filters assignments by recipientIds—departments see only their targeted assignments while admins see all. Created safe migration for `recipient_ids` column. Backward compatible: legacy assignments without recipients remain visible to all departments. Frontend updated with department recipient selection UI (matches ComposeMessage pattern) in AssignmentsPage.
...
- **Message List Complete Redesign (CSS Grid):** Completely rewrote message list using CSS Grid layout for perfect column alignment. Headers (Inbox.tsx) and data rows (MessageListItem.tsx) now share identical `grid-template-columns` definitions (120px for document number, 1fr for subject/content, 150px for sender, 130px for date, 80px for icons). Removed all flex-based hacks, negative margins, and manual alignment tweaks. Implemented separate mobile-first responsive view with stacked layout for small screens. Code is now clean, maintainable, and professional.
