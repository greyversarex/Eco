...
*   **Assignment Restoration (Таъхир):** Assignment creators can restore completed, rejected, or overdue assignments. Restoration resets the assignment status, clears approval, and sets a new deadline of +1 day from restoration date. Restored assignments appear in a dedicated "Таъхир" tab with an orange stamp indicator and badge. Uses `isRestored` and `restoredAt` fields in the assignments table, with `reactivateAssignment` storage method.
