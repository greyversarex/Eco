...
*   **Assignment Restoration (Тахриршуда):** Assignment creators can restore completed, rejected, or overdue assignments. Restoration resets the assignment status, clears approval, and sets a new deadline of +1 day from restoration date. Restored assignments appear in a dedicated "Тахриршуда" tab and are marked with a blue badge. Uses `isRestored` and `restoredAt` fields in the assignments table, with `reactivateAssignment` storage method.
